...
export const metadata: Metadata = {
  title: 'Administración global | Asistencia de laboratorio',
  description: 'Panel global de configuración de la instancia del laboratorio.',
};

export default async function GlobalAdminPage() {
  const context = await requireGlobalAdministrator();
  const dashboard = await getGlobalAdminDashboardData(prisma, context.lab.id);
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14 lg:py-20">
      <section className="grid gap-10 lg:grid-cols-[minmax(0,1.35fr)_minmax(20rem,0.65fr)] lg:items-end">
        <div>
          ...
          <h1 className="max-w-3xl text-5xl leading-[0.98] font-semibold tracking-[-0.045em] text-slate-950 sm:text-6xl lg:text-7xl">
            Gobierno de la instancia
          </h1>
          <p className="mt-7 max-w-2xl text-lg leading-8 text-slate-600">
            Supervise la configuración principal de este laboratorio y delegue
            su administración cotidiana mediante invitaciones seguras.
          </p>
        </div>

        <aside className="border-t-4 border-teal-700 bg-slate-950 p-6 text-white sm:p-8">
      ...
      <section className="mt-14 grid border-y border-slate-300 bg-white sm:grid-cols-2 lg:mt-20">
        <article className="border-b border-slate-200 p-7 sm:border-r sm:border-b-0 sm:p-9">
          <p className="text-sm font-medium text-slate-500">
            Administradores locales activos
          </p>
          <p className="mt-5 text-5xl font-semibold tracking-tight text-slate-950">
            {dashboard.activeManagerCount.toLocaleString('es-CL')}
          </p>
        </article>
        <article className="p-7 sm:p-9">
          ...
          <p className="mt-5 text-5xl font-semibold tracking-tight text-slate-950">
            {dashboard.activeAttendeeCount.toLocaleString('es-CL')}
          </p>
        </article>
      </section>
      ...
      <section className="mt-12 grid gap-8 bg-teal-50 p-7 sm:p-10 lg:grid-cols-[6rem_1fr_auto] lg:items-center">
        ...
        <div>
          ...
          <h2 className="mt-3 text-2xl font-semibold tracking-tight text-slate-950">
            Designar administradores del laboratorio
          </h2>
          <p className="mt-3 max-w-2xl leading-7 text-slate-600">
            Genere una invitación para asignar la gestión cotidiana sin entregar
